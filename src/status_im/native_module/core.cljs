...
(defn change-database-password [key-uid current-password# new-password# callback]
  (log/debug "[native-module] change-database-password")
  (init-keystore
   key-uid
   #(.changeDatabasePassword ^js (status) key-uid current-password# new-password# callback)))
